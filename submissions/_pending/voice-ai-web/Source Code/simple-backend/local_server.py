...
def start_agent():
    ...
    # Register agent_id with custom LLM only when a custom LLM server is configured.
    if agent_response.get("success") and constants.get("LLM_VENDOR") == "custom":
        try:
...
def hangup_agent_route():
    ...
    # Unregister agent from custom LLM only when a custom LLM server is configured.
    if constants.get("LLM_VENDOR") == "custom":
        try:
            llm_url = constants.get("LLM_URL", "")
            if "/chat/completions" in llm_url:
                llm_base = llm_url.rsplit("/chat/completions", 1)[0]
